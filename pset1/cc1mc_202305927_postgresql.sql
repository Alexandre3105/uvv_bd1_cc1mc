...
--Definindo administrador do schema lojas como alexandre 
...
--O nome tem que estar nesse formato 

ALTER TABLE         lojas.clientes
ADD CONSTRAINT      cc_clientes_nome
CHECK               (nome ~ '^[A-Za-z]+$');

--O email tem que estar nesse formato

ALTER TABLE 		lojas.clientes
ADD CONSTRAINT 		cc_clientes_email
CHECK 				(email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$');

--O cliente_id tem que ser maior ou igual a zero
...
--Os telefones tem que estar nesse formato
...
--O endereco_fisico nao pode ser nulo ao mesmo tempo que o endereco_web e vice versa

ALTER TABLE 		lojas.lojas 
ADD CONSTRAINT 		cc_lojas_endereco
CHECK				(endereco_fisico IS NOT NULL OR endereco_web IS NOT NULL);

--A latitude tem que estar entre esses valores
...
--A longitude tem que estar entre esses valores

ALTER TABLE 		lojas.lojas 
ADD CONSTRAINT 		cc_lojas_longitude
CHECK				(longitude BETWEEN -180 AND 180 );

--O loja_Id tem que ser maior ou igual a zero
...
--O preco unitario tem que ser maior ou igual a zero
...
--O produto_id tem que ser maior ou igual a zero 
...
--O pedido_id tem que ser maior ou igual a zero

ALTER TABLE 		lojas.pedidos 
ADD CONSTRAINT 		cc_pedidos_pedido_id
CHECK				(pedido_id >= 0);

--O status dos pedidos so pode conter esses dados
...
--O estoque_id tem que ser maior ou igual a zero 
...
--A quantidade tem que ser maior ou igual a zero 
...
--O envio_id tem que ser maior ou igual a zero
...
--O status dos envios so pode conter esses dados
